...
def draw_board(board, player_score, ai_score):
    ...
    player_label = score_font.render(f"Player: {player_score or "^⁠_⁠^"}", 1, PINK)
    ai_label = score_font.render(f"AI: {ai_score or "^⁠_⁠^"}", 1, CYAN)
...
def display_end_screen(player_score, ai_score, winner_message):
    ...
    player_score_label = score_font.render(f"Player Stars: {player_score}", 1, BLACK)
    ai_score_label = score_font.render(f"AI Stars: {ai_score}", 1, BLACK)
...
while not game_over:
    ...
    while not single_game_over:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION:
                ...
                pos_x = event.pos[0]
                if turn == PLAYER:
                    pygame.draw.circle(screen, PINK, (pos_x, int(SQUARESIZE / 2)), RADIUS)
                pygame.display.update()
                ...
                if turn == PLAYER:
                    ...
                    if is_valid_location(board, col):
                        ...
                        if winning_move(board, PLAYER_PIECE):
                            label = font.render("You win this round!", 1, PINK)
        ...
        # AI Turn
        if turn == AI and not single_game_over:
            ...
            if is_valid_location(board, col):
                ...
                if winning_move(board, AI_PIECE):
                    label = font.render("AI wins this round!", 1, CYAN)
                    screen.blit(label, (40, 10))
                    ai_score += "* "
                    single_game_over = True
    ...
    if player_score == "* * ":
        ...
        winner = "It's a Draw!"
        if len(player_score) > len(ai_score):
            winner = "Player Wins the Game!"
        elif len(ai_score) > len(player_score):
            winner = "AI Wins the Game!"
        display_end_screen(player_score, ai_score, winner)
        game_over = True
